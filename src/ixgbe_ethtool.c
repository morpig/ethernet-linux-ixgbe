			      ...
			      u8 *data)
{
	...
	case ETH_SS_STATS:
		...
		for (i = 0; i < adapter->num_vfs; i++) {
			snprintf(p, ETH_GSTRING_LEN, "vf%u_rx_packets", i);
			p += ETH_GSTRING_LEN;
			snprintf(p, ETH_GSTRING_LEN, "vf%u_rx_bytes", i);
			p += ETH_GSTRING_LEN;
			snprintf(p, ETH_GSTRING_LEN, "vf%u_tx_packets", i);
			p += ETH_GSTRING_LEN;
			snprintf(p, ETH_GSTRING_LEN, "vf%u_tx_bytes", i);
			p += ETH_GSTRING_LEN;
			snprintf(p, ETH_GSTRING_LEN, "vf%u_mc_packets", i);
			p += ETH_GSTRING_LEN;
		}
...
static int ixgbe_setup_desc_rings(struct ixgbe_adapter *adapter)
{
	...
	tx_ring->dev = ixgbe_pf_to_dev(adapter);
	...
	rx_ring->dev = ixgbe_pf_to_dev(adapter);
			  ...
			  struct ethtool_wolinfo *wol)
{
	...
	if (ixgbe_wol_exclusion(adapter, wol) ||
	    !device_can_wakeup(ixgbe_pf_to_dev(adapter)))
		return;
...
static int
ixgbe_get_keee_fw(struct ixgbe_adapter *adapter, struct ethtool_keee *kedata)
{
